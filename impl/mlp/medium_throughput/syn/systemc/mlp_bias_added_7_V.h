...
#ifndef _mlp_bias_added_7_V_H_
#define _mlp_bias_added_7_V_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "mlp_bias_added_7_V_memcore.h"

SC_MODULE(mlp_bias_added_7_V) {
...
protected:
    ...
    // buffer instances
    mlp_bias_added_7_V_memcore* buffer[BufferCount];
...
public:
    ~mlp_bias_added_7_V();
    SC_CTOR(mlp_bias_added_7_V) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "mlp_bias_added_7_V_memcore_U_";
            name += szTmp;

            buffer[i] = new mlp_bias_added_7_V_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "mlp_bias_added_7_V.trace");
            // ports
